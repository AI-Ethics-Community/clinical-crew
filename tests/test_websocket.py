...
class TestWebSocketEndpoint:
    ...
    async def test_websocket_connection(self, client, mock_consultation):
        ...
        with patch('app.models.database.MedicalConsultation.get',
            ...
            with client.websocket_connect(f"/api/v1/ws/consultation/{mock_consultation['consultation_id']}") as websocket:
                # Should receive initial connection event
                data = websocket.receive_text()
                message = json.loads(data)

                assert message["event_type"] == "connected"
                assert message["consultation_id"] == mock_consultation["consultation_id"]
                assert message["data"]["status"] == "interrogating"
    ...
    async def test_websocket_client_disconnect(self, client, mock_consultation):
        ...
        with patch('app.models.database.MedicalConsultation.get',
            ...
            assert mock_consultation["consultation_id"] not in ws_manager.active_connections
...
class TestSSEEndpoint:
    ...
    async def test_sse_connection(self, client, mock_consultation):
        ...
        with patch('app.models.database.MedicalConsultation.get',
            ...
            response = client.get(
                f"/api/v1/consultation/{mock_consultation['consultation_id']}/stream",
                stream=True
            )
...
class TestEventEmitter:
    ...
    async def test_emit_consultation_event(self, mock_consultation):
        ...
        # Cleanup
        event_emitter.clear_consultation(mock_consultation["consultation_id"])
    ...
    async def test_multiple_callbacks(self, mock_consultation):
        ...
        event_emitter.on_consultation(mock_consultation["consultation_id"], callback1)
        event_emitter.on_consultation(mock_consultation["consultation_id"], callback2)
...
class TestWebSocketManager:
    ...
    async def test_manager_multiple_connections(self, mock_consultation):
        ...
        # Connect both
        await ws_manager.connect(mock_consultation["consultation_id"], ws1)
        await ws_manager.connect(mock_consultation["consultation_id"], ws2)

        assert len(ws_manager.active_connections[mock_consultation["consultation_id"]]) == 2
        ...
        # Disconnect
        ws_manager.disconnect(mock_consultation["consultation_id"], ws1)
        ws_manager.disconnect(mock_consultation["consultation_id"], ws2)
