...
async def interrogate_patient(
    ...
    questions_data = await general_practitioner.generate_interrogation_questions(
        consultation=state["original_consultation"], patient_context=contexto
    )
    ...
    if consulta_db:
        consulta_db.interrogation_questions = [
            InterrogationQuestion(**q) for q in questions_data.get("questions", [])
        ]
        consulta_db.update_status("interrogating")
        consulta_db.add_trace(
            "interrogate_patient",
    ...
    state["status"] = (
        "interrogating"
        if not questions_data.get("can_proceed", False)
        else "evaluating"
    )
...
async def evaluate_initial_consultation(
    state: MedicalConsultationState,
) -> MedicalConsultationState:
    """
    ...
    try:
        await event_emitter.emit(
            GPEvaluatingEvent(
                consulta_id=consulta_id,
                data={"phase": "evaluation", "message": "GP evaluating consultation"},
            )
        )
        ...
        if state.get("user_responses"):
            node_logger.debug(
                f"Applying user responses: {list(state['user_responses'].keys())}"
            )
        ...
        node_logger.info(
            f"Evaluation complete: can_answer={evaluacion.can_answer_directly}, specialists={evaluacion.required_specialists}, complexity={evaluacion.estimated_complexity}"
        )
        ...
        if evaluacion.can_answer_directly:
            ...
            node_logger.info(
                f"Will consult {len(evaluacion.required_specialists)} specialists"
            )
            state["status"] = "interconsulting"

        elapsed = time.time() - start_time
        node_logger.info(
            f"=== FINALIZANDO NODO: Evaluate Initial Consultation ({elapsed:.2f}s) ==="
        )
        ...
        elapsed = time.time() - start_time
        node_logger.error(
            f"Error in evaluate_initial_consultation after {elapsed:.2f}s: {str(e)}",
            exc_info=True,
        )
...
async def generate_interconsultations(
    state: MedicalConsultationState,
) -> MedicalConsultationState:
    """
    ...
    try:
        ...
        # Safely construct evaluation with defaults for missing fields (for backward compatibility)
        eval_data = state.get("general_evaluation") or {}
        evaluacion = GeneralEvaluation(
            can_answer_directly=eval_data.get("can_answer_directly", False),
            required_specialists=eval_data.get("required_specialists", []),
            reasoning=eval_data.get("reasoning", "Interconsultation required"),
            estimated_complexity=eval_data.get("estimated_complexity", 0.7),
        )
        contexto = PatientContext(**state["patient_context"])

        node_logger.info(
            f"Generating interconsultations for {len(evaluacion.required_specialists)} specialists"
        )
        interconsultations = []

        for idx, specialty in enumerate(evaluacion.required_specialists, 1):
            node_logger.info(
                f"[{idx}/{len(evaluacion.required_specialists)}] Generating interconsultation for: {specialty}"
            )

            await event_emitter.emit(
                InterconsultationCreatedEvent(
                    consulta_id=consulta_id, data={"specialty": specialty}
                )
            )
            ...
            node_logger.debug(
                f"Interconsultation created for {specialty}: {interconsultation.id}"
            )
        ...
        if consulta_db:
            ...
            consulta_db.add_trace(
                "generate_interconsultations", {"count": len(interconsultations)}
            )
            await consulta_db.save()
        ...
        elapsed = time.time() - start_time
        node_logger.info(
            f"=== FINALIZANDO NODO: Generate Interconsultations ({elapsed:.2f}s, {len(interconsultations)} created) ==="
        )
        ...
        elapsed = time.time() - start_time
        node_logger.error(
            f"Error in generate_interconsultations after {elapsed:.2f}s: {str(e)}",
            exc_info=True,
        )
...
async def execute_specialists(
    ...
    async def process_interconsulta(
        interconsulta_data: Dict[str, Any],
    ) -> Dict[str, Any]:
        interconsultation = InterconsultationNote(**interconsulta_data)

        await event_emitter.emit(
            SpecialistStartedEvent(
                consulta_id=consulta_id, data={"specialty": interconsultation.specialty}
            )
        )
        ...
        async def on_tool_start(tool_name: str, specialty: str):
            await event_emitter.emit(
                ToolStartedEvent(
                    consulta_id=consulta_id,
                    data={"tool": tool_name, "specialty": specialty},
                )
            )
        ...
        async def on_source_found(source):
            await event_emitter.emit(
                SourceFoundEvent(
                    consulta_id=consulta_id, data={"source": source.model_dump()}
                )
            )
...
async def create_direct_response(
    state: MedicalConsultationState,
) -> MedicalConsultationState:
    """
    ...
    try:
        ...
        # Safely construct GeneralEvaluation with defaults for missing fields
        eval_data = state.get("general_evaluation") or {}
        general_evaluation = GeneralEvaluation(
            can_answer_directly=eval_data.get("can_answer_directly", True),
            required_specialists=eval_data.get("required_specialists", []),
            reasoning=eval_data.get("reasoning", "Direct response generated by GP"),
            estimated_complexity=eval_data.get("estimated_complexity", 0.3),
        )
        ...
        final_response = response_data.get(
            "final_response", general_evaluation.reasoning
        )
        general_summary = response_data.get(
            "general_summary", general_evaluation.reasoning
        )
        management_plan = response_data.get("management_plan")
        followup = response_data.get("recommended_followup")

        node_logger.debug(
            f"Generated direct response length: {len(final_response)} chars"
        )
        ...
        if consulta_db:
            ...
            consulta_db.add_trace(
                "create_direct_response_completed", {"expediente_generated": True}
            )
            await consulta_db.save()
        ...
        elapsed = time.time() - start_time
        node_logger.info(
            f"=== FINALIZANDO NODO: Create Direct Response ({elapsed:.2f}s) ==="
        )
        ...
        elapsed = time.time() - start_time
        node_logger.error(
            f"Error in create_direct_response after {elapsed:.2f}s: {str(e)}",
            exc_info=True,
        )
...
async def integrate_responses(
    ...
    if consulta_db:
        consulta_db.update_status("integrating")
        consulta_db.add_trace(
            "integrate_responses_start",
            {"counter_referrals_count": len(state["counter_referrals"])},
        )
        await consulta_db.save()
    ...
    interconsultations = [
        InterconsultationNote(**ic) for ic in state["interconsultations"]
    ]
    ...
    expediente_text = notas_service.generar_expediente_completo(
        ...
        management_plan=response_data.get("management_plan") or [],
...
def should_consult_specialists(state: MedicalConsultationState) -> str:
    ...
    general_eval = state.get("general_evaluation") or {}
    can_answer = general_eval.get("can_answer_directly", False)
    required_specialists = general_eval.get("required_specialists", [])

    node_logger.info(
        f"Routing decision: can_answer_directly={can_answer}, required_specialists={required_specialists}"
    )

    if can_answer:
        node_logger.info(
            "→ Routing to direct_response (GP can answer without specialists)"
        )
        return "direct_response"
    else:
        node_logger.info(
            f"→ Routing to generate_interconsultations ({len(required_specialists)} specialists needed)"
        )
        return "generate_interconsultations"


def create_workflow():
    """Create the medical consultation workflow graph and return compiled app."""
    ...
    workflow.add_conditional_edges(
        "interrogate", should_interrogate, {"interrogate": END, "evaluate": "evaluate"}
    )
...
def create_workflow_from_evaluation():
    """Create workflow that starts from evaluation phase (after interrogation) and return compiled app."""
    workflow = StateGraph(MedicalConsultationState)
